...
const updateGraphsTabOpen = actionCreator('graphsTab.isOpen')<
  | { value: true; currentTab: "Graphs"; } 
  | { value: false; currentTab: "Torrents"; }
>();
...
const reducer = produce((draft: UiState, action: UiActions) => {
  switch (action.type) {
    ...
    case 'graphsTab.isOpen':
      draft.graphsTab.isOpen = action.payload.value;
      draft.graphsTab.currentTab = action.payload.currentTab;
      break;
